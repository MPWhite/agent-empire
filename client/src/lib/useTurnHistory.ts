...
export function useTurnHistory(
  ...
  initialTurnHistory?: TurnHistoryEntry[] | null,
): TurnHistory {
  ...
  // Seed from initial turn history on load
  useEffect(() => {
    if (!initialTurnHistory || initialTurnHistory.length === 0 || seededRef.current) return;
    seededRef.current = true;

    historyRef.current = initialTurnHistory;
    lastTurnRef.current = initialTurnHistory[initialTurnHistory.length - 1].turn;
    setDeltas({});
  }, [initialTurnHistory]);
